	...
	function generate() {
		...
		for ($frame_count = 1;$frame_count <= $this->frame_number; $frame_count++) {
			...
			if ($random_font) {
				...
				$height = (int) (@imagefontheight($random_font)*$this->font_size/4)+($this->random_y_factor*2)-13;
			} else {
				$x= (imagefontwidth($this->font_size)+$this->text_space)*$this->string_length;
				$height = @imagefontheight($this->font_size)+10+($this->random_y_factor*2)-13;
			}
			$width = $x + 20;
			...
			if ($this->frame_number > 1) {
				...
				$rand_filename = $this->temp_dir.md5(microtime().getenv('HTTP_X_FORWARDED_FOR')).mt_rand(1, 999).'.txt';
