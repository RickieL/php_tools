...
class PageExcel extends APageFactory
{
	...
	public function output($filename = 'noname.xls')
	{
		...
		echo '<html><head><meta http-equiv="Content-Type" content="application/vnd.ms-excel; charset=' . $this->app->cfg['page']['charset'] . '" /></head><body><table>';
		if (! empty($this->value))
		{
			$index = 0;
				...
				foreach ($this->header as $field => $value)
				{
					...
					if ($attrs = $row[$field . ' attr'])
					{
						...
						else
						{
							...
							{
								echo ' ' . $attrName . '="' . str_replace('"', '\"', $attrValue) . '"';
								if ($attrName == 'colspan')
								{
