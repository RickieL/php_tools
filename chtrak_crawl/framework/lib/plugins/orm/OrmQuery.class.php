...
class OrmQuery
{
	 ...
	 * @return array
	 */
	public function getArray($sql = NULL, $offset = 0, $count = -1, $fetchMode = _ORM_FETCH_ASSOC)
	{
		($sql === NULL) && $sql = $this->asSql('select');
	 ...
	 */
	public function call($procedureName)
	{
	 ...
	 * @return string
	 */
	public function asSql($type = 'select')
	{
		$this->sql['mode'] = $type;
	...
	public function clear($part = '')
	{
		...
		{
			$this->sql = array('mode' => 'select', 'table' => array(), 'field' => array(), 'value' => array(), 'where' => array(), 'group' => array(), 'order' => array());
		}
		return $this;
	}
	...
	public function addTable($tableName, $alias = '')
	{
		...
		$this->sql['table'][] = array('name' => $tableName, 'alias' => empty($alias) ? $tableName : $alias);
		return $this;
	}
	 ...
	 * @return OrmQuery
	 */
	public function addField($fieldName, $alias = '', $opt = '', $fieldType = _ORM_DT_AUTO)
	{
		...
		$this->sql['field'][] = array('name' => $fieldName, 'alias' => empty($alias) ? $fieldName : $alias, 'opt' => $opt, 'type' => $fieldType);
		return $this;
	}
	 ...
	 * @return OrmQuery
	 */
	public function addValue($fieldName, $fieldValue, $fieldType = _ORM_DT_AUTO)
	{
		$fieldName = trim($fieldName);
		if (empty($fieldName))
		{
		...
		$this->sql['value'][] = array('name' => $fieldName, 'value' => $fieldValue, 'type' => $fieldType);
		return $this;
	}
	 ...
	 * @return OrmQuery
	 */
	public function addWhere($fieldName, $value = '', $opt = _ORM_OP_EQ, $fieldType = _ORM_DT_AUTO, $logical = _ORM_OP_AND)
	{
		$fieldName = trim($fieldName);
		if (empty($fieldName))
		{
		...
		$this->sql['where'][] = array('name' => $fieldName, 'value' => $value, 'opt' => $opt, 'type' => $fieldType, 'logical' => $logical);
		return $this;
	}
	 ...
	 * @return OrmQuery
	 */
	public function addOrderBy($fieldName, $orderBy = _ORM_OP_ASC)
	{
		$fieldName = trim($fieldName);
		if (empty($fieldName))
		{
		...
		$this->sql['order'][] = array('name' => $fieldName, 'order' => $orderBy);
		return $this;
	}
