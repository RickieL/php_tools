...
class MssqlOrmParser
{
	...
	public function connect()
	{
		...
		{
			$this->connect = mssql_pconnect($this->params['host'], $this->params['user'], $this->params['password']);
		}
		else
		{
			$this->connect = mssql_connect($this->params['host'], $this->params['user'], $this->params['password']);
		}
	}
	 ...
	 * @access private
	 */
	public function parseSqlTable($tables, $withAlias = false)
	{
		if (empty($tables))
		{
	...
	public function fetch($dataset, $fetchMode = _ORM_FETCH_ASSOC)
	{
		switch ($fetchMode)
		{
			case _ORM_FETCH_ASSOC:
				$result = mssql_fetch_assoc($dataset);
				break;
			case _ORM_FETCH_NUM:
				$result = mssql_fetch_row($dataset);
				break;
			case _ORM_FETCH_BOTH:
				$result = mssql_fetch_array($dataset);
				break;
			case _ORM_FETCH_OBJ:
				$result = mssql_fetch_object($dataset);
				break;
		}
		return $result;
	}
	 ...
	 * @return string
	 */
	public function call($procedureName, $args = null)
	{
		return false;
	}
	...
	{
		throwException('SQL执行错误: <b>' . $sql . '</b><br /> 错误代码: <b>' . mssql_get_last_message() . '</b><br /> 错误信息: <b>' . mssql_get_last_message() . '</b>', 4001);
	}
}
